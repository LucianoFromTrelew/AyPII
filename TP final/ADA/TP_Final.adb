...
--NIVEL 5
   ...
   function ingresoIncorrecto (msj: in string) return boolean is
	...
	--PRE: -
	--POS: obtenerPatente = P. P es una patente válida
	--Excepciones: -}
      pat: tipoClaveAuto;
      cad: Unbounded_String;
	  OK: boolean;
	begin
		...
		pat.num := enteroEnRango("Ingrese números de la patente", 0, 999);
	...
	begin
		CLS;
		año := enteroEnRango("Ingrese el año entre", año_ACTUAL, 9999);
...
--NIVEL 4
	...
	begin
		...
		loop
			CLS;
			tel := mayorCeroLL("Ingrese número de teléfono celular del cliente (característica de la ciudad de Trelew)");
	...
	procedure cambioDueño(clientes: in out arbolClientes.tipoArbol; pat: in tipoClaveAuto; iDueño: in out infoVehiculos) is
	...
	begin
		...
		mostrarMensaje("Ingrese DNI del nuevo propietario (registrado en el sistema) del vehículo");
	...
	begin
		...
		while OK loop
			...
			put_line("Propietario del vehículo: " & long_long_integer'image(infoVehi.dueño));
			put_line("Patente del vehículo: " & pat.letras & integer'image(pat.num));
	...
	exception
		when listaMant.listaVacia => put_line("No se registró ningún mantenimiento sobre el vehículo de patente " & pat.letras & integer'image(pat.num));
	end mostrarMant;
...
--NIVEL 3
	...
	begin
		...
		k := KAutoNum(modelos);
		mostrarMensaje("El modelo " & To_String(info.nomMod) & " tiene el código" & integer'image(k));
	...
	begin
		pedirDNIexistente(clientes, iVehi.dueño, iClien);
      
		pedirCod(iVehi.cod, modelos, iModelos);
      
		pedirPatenteNueva(vehiculos, pat);
		
		pedirAño(iVehi.año, MIN_AÑO);
	...
	begin
		If (not(esVacia(modelos))) then
			...
			mostrarMensaje("No hay ningún modelo cargado");
			raise salir;
	...
	begin
		...
		if (not(esVacia(info.mantenimientos))) then
			...
			mostrarMensaje("No hay ningún servicio realizado sobre el vehículo");
			raise salir;
	...
	exception
      when listaVehiculos.listaVacia => 
         mostrarMensaje("El cliente no solicitó ningún servicio");
		 raise salir;
	...
	begin
		...
		if(not(esVacia(i.calendario))) then
			...
			mostrarMensaje("El modelo de código " & Integer'Image(cod) & " no tiene etapas de mantenimientos cargadas");
			raise salir;
   ...
   begin
      if(not(esVacio(clientes)))then
		...
		while(not(esVacia(qAux))) loop
         ...
         mostrarMensaje("No hay ningún cliente cargado");
         raise salir;
      end if;
	exception
	...
	begin
		while(not(esVacia(q))) loop
			...
			put_line("Teléfono: " & Long_Long_Integer'image(i.tel));
...
--NIVEL 2	
	...
	exception
		...
		when arbolVehiculos.claveExiste =>
			mostrarMensaje("Vehículo ya existe");
	end altaVehiculos;
	...
	--Excepciones:	-
		...
		i: infoModelos;
	begin
		mostrarMensaje("Se eliminarán todos los vehículos registrados de ese modelo");
	...
	--Excepciones:	-
		...
		i: infoModelos;
	begin
		mostrarMensaje("Se eliminarán todos los servicios de la etapa que se ingrese.");
	...
	begin
		...
		mostrarMensaje("Clientes que no registraron ningún mantenimiento en alguno de sus vehículos registrados: ");
...
--NIVEL 1.
   ...
   begin
	...
	begin
		...
		loop
		  mostrarMenuPpal;
		  opc := enteroEnRango("Ingrese su opción asas", 1, 5);
